...
// 本地存储键名
const STORAGE_KNOWN_CARDS = '2005_knownCards';
const STORAGE_CURRENT_LECTURE = '2005_currentLecture';
const STORAGE_CURRENT_INDEX = '2005_currentIndex';
const STORAGE_MEMORY_MODE = '2005_memoryMode'; // 记忆模式存储键
...
function resetAllStoredData() {
    localStorage.removeItem(STORAGE_KNOWN_CARDS);
    localStorage.removeItem(STORAGE_CURRENT_LECTURE);
    localStorage.removeItem(STORAGE_CURRENT_INDEX);
    localStorage.removeItem(STORAGE_MEMORY_MODE);
    ...
    try {
        ...
        function setVoiceAndSpeak() {
            ...
            if (isIOS) {
                ...
                // 如果没找到指定声音，寻找任何美式英语声音
                if (!selectedVoice) {
                    selectedVoice = voices.find(voice => voice.lang === 'en-US');
                }
                
                // iOS设备上的语音参数
                speech.rate = 0.9;     // 稍微放慢速度
